...
As described in the documentations around [the `OAuthTokenRepresentative` protocol](/Sources/OAuthBM/OAuthTokenRepresentative.swift), the `initializeAndSave(request:token:oldToken:) -> ELF<Self>` func is just an initializer that gives you more flexibility than a normal `init`,
...
You do need to save the token into the database as well.
